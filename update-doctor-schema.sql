...
ALTER TABLE doctors DROP COLUMN IF EXISTS sub_specialty;

-- 새 필드 추가
ALTER TABLE doctors ADD COLUMN IF NOT EXISTS aspired_beauty TEXT[] DEFAULT '{}';  -- 추구하는 beauty
ALTER TABLE doctors ADD COLUMN IF NOT EXISTS care_philosophy TEXT;  -- 진료 철학
ALTER TABLE doctors ADD COLUMN IF NOT EXISTS clinical_experience TEXT[] DEFAULT '{}';  -- 임상 경력
ALTER TABLE doctors ADD COLUMN IF NOT EXISTS specialist_experience TEXT[] DEFAULT '{}';  -- 전문의 취득 후 임상 경력
ALTER TABLE doctors ADD COLUMN IF NOT EXISTS specialized_area TEXT[] DEFAULT '{}';  -- 전문 분야 (최대 30글자)
ALTER TABLE doctors ADD COLUMN IF NOT EXISTS languages TEXT[] DEFAULT '{}';  -- 언어 (최대 30글자)

-- 인덱스 추가 (배열 검색 성능 향상)
-- 참고: CareArea, Language 길이 제한(최대 30글자)은 애플리케이션 레벨에서 검증
...
-- ) VALUES (
--   '김철수',
--   ARRAY['자연스러운 아름다움', '개인별 맞춤 미'],
--   '환자와 함께하는 치료',
--   ARRAY['서울대병원 성형외과 10년', '강남세브란스병원 5년'],
--   ARRAY['안면윤곽 수술 200례', '코성형 수술 150례'],
--   ARRAY['안면윤곽', '코성형', '눈성형'],
--   ARRAY['한국어', '영어', '일본어'],
--   'hospital-uuid-here'
-- );
...
-- 예시: 배열 검색 쿼리
-- 특정 care_area를 가진 의사 검색
-- SELECT * FROM doctors WHERE '척추질환' = ANY(specialized_area);

-- 특정 language를 구사하는 의사 검색
-- SELECT * FROM doctors WHERE '영어' = ANY(languages);

-- 여러 care_area를 모두 가진 의사 검색
-- SELECT * FROM doctors WHERE specialized_area @> ARRAY['척추질환', '관절질환'];
...
(
  ...
  '눈가 안티에이징 전문의. 섬세한 시술로 자연스러운 동안 효과를 추구합니다.',
  12
),
-- 의사 2: 이서연
(
  ...
  '11111111-1111-1111-1111-111111111111',
  '턱선 및 안면윤곽 전문의. 비수술적 방법으로 세련된 얼굴형을 만들어드립니다.',
  8
),
-- 의사 3: 박준혁
(
  ...
  '다크서클 치료 전문가. 다양한 원인에 맞는 복합적 치료를 제공합니다.',
  15
),
-- 의사 4: 최유진
(
  ...
  '22222222-2222-2222-2222-222222222222',
  '입가 주름 개선 전문의. 자연스러운 미소를 되찾아드립니다.',
  10
),
-- 의사 5: 정태영
(
  ...
  '안면 윤곽 비수술 전문의. 주사와 레이저로 슬림한 턱선을 만들어드립니다.',
  9
),
-- 의사 6: 한소희
(
  ...
  '33333333-3333-3333-3333-333333333333',
  '눈가 전문의. 동남아시아 환자분들의 진료 경험이 풍부합니다.',
  14
),
-- 의사 7: 강민재
(
...
(
  ...
  '22222222-2222-2222-2222-222222222222',
  '유럽 연수 경험의 피부과 전문의. 다양한 레이저 시술에 전문성을 갖추고 있습니다.',
  7
),
-- 의사 9: 임동훈
(
  ...
  '33333333-3333-3333-3333-333333333333',
  '하안면 윤곽 전문의. 비수술 및 최소침습 시술로 자연스러운 변화를 추구합니다.',
  13
),
-- 의사 10: 오지현
(
  ...
  '11111111-1111-1111-1111-111111111111',
  '눈 주변 안티에이징 전문의. 중동 환자분들의 진료 경험이 풍부합니다.',
  6
)
ON CONFLICT (id) DO NOTHING;
